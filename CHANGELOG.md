...
- `InEnum` constraint — `#[InEnum(MyEnum::class)]` validates a value against an enum. For `BackedEnum`, the value must match a backing value (string-backed → `StringType`, int-backed → `Integer`). For pure (non-backed) `UnitEnum`, the value must match a case name (used on `StringType`). No `EnumString` / `EnumInt` base classes are introduced; `InEnum` on a `StringType` or `Integer` subclass is sufficient. Constructed with a non-enum, non-existent, or interface class name, `InEnum` throws `\LogicException` per the constraint constructor invariants.
